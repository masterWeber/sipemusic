<template>
  <div class="photos-page">
    <div class="photos-page__header">
      <h1 class="photos-page__title">Фотографии участников</h1>
      <button class="photos-page__add-btn" @click="openUpload">+ Добавить фото</button>
    </div>

    <div v-if="loading" class="photos-page__loading">Загрузка...</div>

    <div v-else-if="photos.length === 0" class="photos-page__empty">
      Фотографий пока нет. Нажмите «Добавить фото», чтобы загрузить первую.
    </div>
    ...
    <div v-else class="photos-grid">
      ...
      >
        <div class="photo-card__image-wrap">
          <img :src="photo.image" :alt="photo.name || `Фото ${index + 1}`" class="photo-card__image">
        </div>
        <div class="photo-card__info">
          <span class="photo-card__index">#{{ index + 1 }}</span>
          <span class="photo-card__name">{{ photo.name || 'Без названия' }}</span>
        </div>
        <div class="photo-card__actions">
          <button
            class="photo-card__btn photo-card__btn--up"
            :disabled="index === 0"
            @click="moveUp(photo.id)"
            aria-label="Переместить вверх"
          >◀</button>
          <button
            class="photo-card__btn photo-card__btn--down"
            :disabled="index === photos.length - 1"
            @click="moveDown(photo.id)"
    ...
    <Teleport to="body">
      <div v-if="uploadModalOpen" class="modal-overlay" @click.self="closeUpload">
        <div class="modal-card modal-card--upload">
          <h2 class="modal-card__title">{{ cropping ? 'Обрезка фото' : 'Загрузка фото' }}</h2>
          ...
          <div v-else class="crop-area">
            <div class="crop-area__field">
              <label class="crop-area__label">Название</label>
              <input v-model="uploadName" class="crop-area__input" placeholder="Вводите название">
            </div>
    ...
    <Teleport to="body">
      <div v-if="deleteModalOpen" class="modal-overlay" @click.self="deleteModalOpen = false">
        <div class="modal-card">
          <h2 class="modal-card__title">Удалить фото</h2>
          <p class="modal-card__text">
            Вы уверены, что хотите удалить фото <strong>{{ deletingPhoto?.name }}</strong>?
          </p>
          <div class="modal-card__actions">
            <button class="modal-card__btn modal-card__btn--cancel" @click="deleteModalOpen = false">Отмена</button>
            <button class="modal-card__btn modal-card__btn--delete" :disabled="deleting" @click="deletePhoto">
...
async function fetchPhotos() {
  loading.value = true
  try {
    photos.value = await $fetch('/api/about-photos')
  } finally {
    loading.value = false
  }
}
...
async function moveUp(id) {
  ...
  await $fetch('/api/about-photos/reorder', {
    method: 'PUT',
    body: { ids: newPhotos.map(p => p.id) },
  })
}
...
async function deletePhoto() {
  ...
  deleting.value = true
  try {
    await $fetch(`/api/about-photos/${deletingPhoto.value.id}`, { method: 'DELETE' })
